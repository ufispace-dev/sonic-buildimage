...
EC_P_REG = {
    "reg": 0x231D,
    "major": {"mask": 0b11111111, "pos": 0}
}

EC_D_REG = {
    "reg": 0x231E,
    "minor": {"mask": 0b11111111, "pos": 0}
}
...
CPLD_SYSFS = {
    "CPLD1": {"major": "cpld1_major_ver", "minor": "cpld1_minor_ver", "build": "cpld1_build_ver"},
    "CPLD2": {"major": "cpld2_major_ver", "minor": "cpld2_minor_ver", "build": "cpld2_build_ver"},
    "CPLD3": {"major": "cpld3_major_ver", "minor": "cpld3_minor_ver", "build": "cpld3_build_ver"},
    "CPLD4": {"major": "cpld4_major_ver", "minor": "cpld4_minor_ver", "build": "cpld4_build_ver"},
    "FPGA": {"major": "fpga_major_ver", "minor": "fpga_minor_ver", "build": "fpga_build_ver"},
}

BMC_CMDS = {
    "BMC": "bash -c 'tmp=$(ipmitool raw 0x6 0x1 2>/dev/null) && r=($(echo \"$tmp\" | cut -d \" \" -f 4,5,16,15,14)) && echo ${r[0]}.${r[1]}.${r[4]}.${r[3]}${r[2]}'",
}

BIOS_VERSION_PATH = "/sys/class/dmi/id/bios_version"

COMPONENT_LIST_ALPHA= [
   ("CPU_CPLD", "CPU CPLD"),
   ("CPLD1", "CPLD 1"),
   ("CPLD2", "CPLD 2"),
   ("CPLD3", "CPLD 3"),
...
class Component(ComponentBase):
    ...
    def _get_ec_version(self):
        try:
            with open(IOPORT_PATH, "rb") as f:
                f.seek(EC_P_REG.get("reg"), 0)
                reg = int.from_bytes(f.read(1), signed=False)
                mask = EC_P_REG["major"]["mask"]
                pos = EC_P_REG["major"]["pos"]
                major = (reg & mask) >> pos

                f.seek(EC_D_REG.get("reg"), 0)
                reg = int.from_bytes(f.read(1), signed=False)
                mask = EC_D_REG["minor"]["mask"]
                pos = EC_D_REG["minor"]["pos"]
                minor = (reg & mask) >> pos

                f.seek(EC_T_REG.get("reg"),0)
                reg = int.from_bytes(f.read(1), signed=False)
                mask = EC_T_REG["build"]["mask"]
                pos = EC_T_REG["build"]["pos"]
                build = (reg & mask) >> pos
    ...
    def get_firmware_version(self):
        ...
        elif "EC" in self.name:
            fw_version = self._get_ec_version()
