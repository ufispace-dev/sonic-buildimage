...
extern int board_i2c_cpld_write_custom(unsigned short cpld_addr, u8 reg, u8 value);
...
PDDF_DATA_ATTR(fan_present    , S_IRUGO, show_sysstatus_data, NULL, PDDF_UCHAR, sizeof(u8), NULL, NULL);
PDDF_DATA_ATTR(psu_status     , S_IRUGO, show_sysstatus_data, NULL, PDDF_UCHAR, sizeof(u8), NULL, NULL);
PDDF_DATA_ATTR(led_ctrl1      , S_IWUSR|S_IRUGO, show_sysstatus_data, store_sysstatus_data, PDDF_UCHAR, sizeof(u8), NULL, NULL);
PDDF_DATA_ATTR(led_ctrl2      , S_IWUSR|S_IRUGO, show_sysstatus_data, store_sysstatus_data, PDDF_UCHAR, sizeof(u8), NULL, NULL);
PDDF_DATA_ATTR(led_status     , S_IRUGO, show_sysstatus_data, NULL, PDDF_UCHAR, sizeof(u8), NULL, NULL);
...
static struct attribute *sysstatus_data_attributes[] = {
    ...
    &attr_fan_present.dev_attr.attr,
    &attr_psu_status.dev_attr.attr,
    &attr_led_ctrl1.dev_attr.attr,
    &attr_led_ctrl2.dev_attr.attr,
    &attr_led_status.dev_attr.attr,
    NULL
};
...
ssize_t show_sysstatus_data(struct device *dev, struct device_attribute *da, char *buf)
{
    ...
    {
        status = board_i2c_cpld_read_custom( sysstatus_addr_attrs->devaddr, sysstatus_addr_attrs->offset);
    }
...
ssize_t store_sysstatus_data(struct device *dev, struct device_attribute *da, const char *buf, size_t count)
{
    ...
    else
    {
        ...
        status = board_i2c_cpld_write_custom(sysstatus_addr_attrs->devaddr, sysstatus_addr_attrs->offset, reg_val);
