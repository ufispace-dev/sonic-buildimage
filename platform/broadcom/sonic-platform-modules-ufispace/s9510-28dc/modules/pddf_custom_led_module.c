 ...
 * This driver is based on the pddf_led_module.c
 */
...
ssize_t get_status_led(struct device_attribute *da)
{
    ...
    if (strcmp(ops_ptr->attr_devtype, "cpld") == 0) {
        cpld_type = 1;
        sys_val = board_i2c_cpld_read_custom(ops_ptr->swpld_addr, ops_ptr->swpld_addr_offset);
...
ssize_t set_status_led(struct device_attribute *da)
{
    ...
    if (strcmp(ops_ptr->data[cur_state].attr_devtype, "cpld") == 0) {
        ret = board_i2c_cpld_write_custom(ops_ptr->swpld_addr, ops_ptr->swpld_addr_offset, new_val);
        read_val = board_i2c_cpld_read_custom(ops_ptr->swpld_addr, ops_ptr->swpld_addr_offset);
    } else if (strcmp(ops_ptr->data[cur_state].attr_devtype, "fpgai2c") == 0) {
...
MODULE_DESCRIPTION("pddf custom led driver");
MODULE_LICENSE("GPL");
