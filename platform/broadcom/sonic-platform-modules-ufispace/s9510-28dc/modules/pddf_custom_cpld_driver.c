...
EXPORT_SYMBOL(board_i2c_cpld_read_new_custom);
...
EXPORT_SYMBOL(board_i2c_cpld_write_new_custom);
...
EXPORT_SYMBOL(board_i2c_cpld_read_custom);
...
EXPORT_SYMBOL(board_i2c_cpld_write_custom);
...
ssize_t regval_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    if (pddf_cpld_data.reg_addr!=0)
        len = sprintf(buf, "0x%2.2x\n", board_i2c_cpld_read_custom(client->addr, pddf_cpld_data.reg_addr));
    else
        len = sprintf(buf, "xx\n");
...
MODULE_DESCRIPTION("pddf custom board_cpld driver api");
MODULE_LICENSE("GPL");
